...
const LANG_FILTER = parseList(process.env.PRERENDER_LANGS);
const PAGE_FILTER = parseList(process.env.PRERENDER_PAGES);
...
const browser = await puppeteer.launch(launchOptions);
  const routes = [];
  const pagesToRender = PAGES.filter((page) => !PAGE_FILTER || PAGE_FILTER.includes(page.path));
  for (const page of pagesToRender) {
    const langsToRender = page.langs.filter((lang) => !LANG_FILTER || LANG_FILTER.includes(lang));
