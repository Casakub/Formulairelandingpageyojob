...
export function TranslationStatistics({ onBack, onSelectLanguage }: Props) {
  ...
  const languagesProgress = LANGUAGES.map(lang => ({
    ...lang,
    total: 25, // 25 questions
    translated: lang.code === 'fr' ? 25 : Math.floor(Math.random() * 15) + 10,
    validated: lang.code === 'fr' ? 25 : Math.floor(Math.random() * 10) + 5,
    autoGenerated: lang.code === 'fr' ? 0 : Math.floor(Math.random() * 8) + 2,
  }));

  // Calculate global statistics
  const globalStats = {
    totalTranslations: 25 * (LANGUAGES.length - 1), // 25 questions × 7 langues cibles
    completed: languagesProgress.reduce((acc, lang) => acc + lang.translated, 0) - 25, // -25 for FR
    validated: languagesProgress.reduce((acc, lang) => acc + lang.validated, 0) - 25,
    autoGenerated: languagesProgress.reduce((acc, lang) => acc + lang.autoGenerated, 0),
  };

  const completionRate = (globalStats.completed / globalStats.totalTranslations) * 100;
  const validationRate = (globalStats.validated / globalStats.totalTranslations) * 100;
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div>
          <h2 className="text-slate-900">📊 Statistiques de traduction</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-2 border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
          ...
          <CardContent>
            ...
            <p className="text-sm text-slate-600">
              {globalStats.validated} traductions validées
            </p>
        ...
        <Card className="border-2 border-cyan-200 bg-gradient-to-br from-cyan-50 to-blue-50">
          ...
          <CardContent>
            <p className="text-3xl text-cyan-600 mb-1">{LANGUAGES.length}</p>
            <p className="text-sm text-slate-600">
              1 source + {LANGUAGES.length - 1} cibles
            </p>
        ...
        <Card className="border-slate-200">
          <CardContent className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm text-slate-600 mb-1">Questions complètes</p>
                <p className="text-2xl text-slate-900 mb-1">
        ...
        <CardContent className="space-y-2">
          {completionRate < 50 && (
            <p className="text-sm text-orange-800">
              • Priorisez les traductions EN, DE et ES pour couvrir 70% du marché européen
            </p>
          )}
          {globalStats.autoGenerated > 10 && (
            <p className="text-sm text-orange-800">
              • {globalStats.autoGenerated} traductions automatiques nécessitent une validation manuelle
            </p>
          )}
          {validationRate < 80 && (
            <p className="text-sm text-orange-800">
              • Faites valider les traductions par des native speakers pour garantir la qualité
            </p>
          )}
          {completionRate >= 80 && (
            <p className="text-sm text-green-800">
              ✓ Excellent travail ! Vous êtes prêt à lancer les campagnes dans {Math.floor(completionRate / 12.5)} pays
            </p>
          )}
        </CardContent>
      </Card>
