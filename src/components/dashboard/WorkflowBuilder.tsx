...
export function WorkflowBuilder({ open, onClose, onSave, editingWorkflow }: WorkflowBuilderProps) {
  ...
  const [trigger, setTrigger] = useState<AutomationTrigger>(
    editingWorkflow?.trigger || { type: 'prospect_created', config: {} }
  );
  
  const [conditions, setConditions] = useState<AutomationCondition[]>(
    editingWorkflow?.conditions || []
  );
            ...
            <div key={step.number} className="flex items-center flex-1">
              <div className="flex flex-col items-center flex-1">
                ...
                <div className="mt-2 text-center">
                  <div className={`text-sm ${currentStep === step.number ? 'text-slate-900' : 'text-slate-600'}`}>
                    {step.title}
                  </div>
              ...
              {idx < WIZARD_STEPS.length - 1 && (
                <div className={`h-0.5 flex-1 ${currentStep > step.number ? 'bg-green-500' : 'bg-slate-200'}`} />
              )}
            </div>
          ))}
        </div>
